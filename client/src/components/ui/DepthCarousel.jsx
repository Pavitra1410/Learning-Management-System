...
export default function DepthCarousel({
  ...
  const layout = useCallback(pos => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const brightness = Math.max(0.25, 1 - back * cfg.falloff);
      ...
      if (ov) ov.style.opacity = clamp(back * cfg.falloff * 0.9, 0, 0.6).toFixed(3);
    }
  }, []);
  ...
  return (
    <div
      ...
      aria-label="LMS Quick Find Course Carousel"
      tabIndex={0}
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {renderCard ? (
              renderCard(item, active === i)
            ) : (
              <>
                <img className="depth-carousel__img" src={item.image || item.thumbnail} alt={item.title || item.alt || ''} draggable={false} />
                <span
